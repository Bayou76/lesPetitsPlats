...
function filterAndDisplayRecipesBySelectedItems() {
  ...
  const selectedIngredientSpans = document.querySelectorAll(
  ...
  const selectedApplianceSpans = document.querySelectorAll(
  ...
  const selectedUstensilSpans = document.querySelectorAll(
  ...
  // Créer des ensembles pour stocker les ingrédients, appareils et ustensiles de toutes les recettes affichées
  let displayedIngredients = new Set();
  let displayedAppliances = new Set();
  let displayedUstensils = new Set();
  ...
  for (let i = 0; i < recipes.length; i++) {
    ...
    const recipeIngredients = Array.from(
      card.querySelectorAll(".card-ingredients-list-item-name")
    ).map((ingredient) => ingredient.textContent.trim().toLowerCase());
    ...
    const recipeUstensils = recipe._ustensils.map((ustensil) =>
      ustensil.trim().toLowerCase()
    );
    ...
    if (isRecipeVisible) {
      ...
      // Collecter les ingrédients de la recette
      recipeIngredients.forEach((ingredient) =>
        displayedIngredients.add(ingredient)
      );

      // Collecter l'appareil de la recette
      displayedAppliances.add(recipeAppliance);

      // Collecter les ustensiles de la recette
      recipeUstensils.forEach((ustensil) => displayedUstensils.add(ustensil));
    }
  }
  console.log(displayedAppliances);
  // Mettre à jour les listes de tags avec les éléments des recettes affichées
  updateTagElement(".tag1-list .tag-list-item-name", displayedIngredients);
  updateTagElement(".tag2-list .tag-list-item-name", displayedAppliances);
  updateTagElement(".tag3-list .tag-list-item-name", displayedUstensils);

  // Mettre à jour le compteur de recettes
  app.updateRecipeCount(visibleRecipeCount);
}

function updateTagElement(selector, elements) {
  const tagListItems = document.querySelectorAll(selector);

  tagListItems.forEach((tagListItem) => {
    const tagName = tagListItem.textContent.trim().toLowerCase();

    if (elements instanceof Set) {
      if (!elements.has(tagName)) {
        tagListItem.parentElement.remove();
      } else {
        elements.delete(tagName);
      }
    }
  });
  ...
  if (elements instanceof Set) {
    elements.forEach((newElement) => {
      ...
      listItem.innerHTML = `<span class="tag-list-item-name">${newElement}</span>`;
      document.querySelector(selector.split(" ")[0]).appendChild(listItem);
    });
  }
}